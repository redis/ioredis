# ioredis Maintainer Evaluation Framework
...
- **None**: no runtime difference, unreachable/unsupported input, cosmetic inconsistency, repository-only cleanup, or harmless edge case. Usually close, document, or decline code complexity.
- **Low**: real but narrow and recoverable behavior with a simple workaround and no data, security, command-ordering, or compatibility risk. Merge only when the fix is small and strengthens an invariant.
- **Medium**: supported use fails or produces incorrect behavior for a meaningful subset. Prioritize a bounded fix and regression test.
- **High**: common or important use is broken, released compatibility is seriously affected, commands can be misrouted or reordered, Cluster/Sentinel failover is broken, resources leak, or sensitive data can leak. Require urgent strong validation.
- **Critical**: broadly exploitable security impact, severe data loss, command execution against the wrong target, or systemic production failure requiring coordinated action. Use only with concrete evidence.
...
- Are stale `built/`, wrong worktree/imports, dependency drift, proxies, caches, runtime conditions, unavailable Redis/Docker/sandbox, authentication, ACLs, and service failures excluded?
- Does an equivalent standalone/Sentinel/Cluster, RESP2/RESP3, string/Buffer, callback/Promise, pipeline/transaction, subscriber, or public-export path differ?
...
- **Plausible but unproven**: The code path is possible, but realistic reach, frequency, consequence, Redis behavior, or demand is missing.
- **Already covered**: A reasonable supported workflow already satisfies the outcome.
- **Unsupported**: The outcome is outside the ioredis public contract or belongs at the Redis server, connector, command transformer, or caller-owned layer.
...
- **Unsupported use case**: the desired outcome lies outside the ioredis public contract or belongs at the Redis server, connector, command transformer, application, or other caller-owned layer. Do not expand the core API merely to make it possible.
...
- **Prioritize**: confirmed medium-or-higher impact or important invariant with no safe workaround.
...
3. **Placement**: the invariant is enforced once at the owning layer instead of duplicating existing functionality, patching locally, or moving Redis server, connector, or caller-owned policy into the core client.
4. **Consistency**: equivalent standalone/Sentinel/Cluster, RESP2/RESP3, string/Buffer, callback/Promise, pipeline/transaction, subscriber, command transformer, and public-export paths stay aligned.
...
6. **Compatibility**: released exports, TypeScript declarations, command signatures, Redis protocol behavior, command results, option defaults, and error behavior are preserved or intentionally migrated.
...
- List side effects before and after that point: listeners, promises/tasks, streams, sockets, Redis connections, timers, files, locks, caches, queues, state, persistence, and diagnostics.
...
- Can the requested outcome already be achieved through configuration, composition, duplicated clients, callbacks, command transformers, custom connectors, scripts, or caller-owned code?
...
- Would clearer error text, TypeScript/JSDoc, README guidance, or validation prevent misuse without runtime complexity?
- Can a failing test reveal a smaller correct change?
- Is a new public option compensating for an internal ownership problem?
- Can the same result be achieved in the command transformer, connector, topology owner, or state owner instead of every caller?
- Is the proposed core behavior actually Redis server, deployment, or application-specific policy that belongs at another layer?
