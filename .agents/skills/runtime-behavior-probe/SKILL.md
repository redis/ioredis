...
- For `ioredis`, treat this skill as a disposable-probe workflow, not a repository implementation workflow.
...
- For disposable probes in `ioredis`, do not modify `examples/**`, `lib/**`, `test/**`, any `package.json`, `README.md`, build config, or generated files unless the user explicitly asks for a reusable repository artifact or implementation change.
...
- In `ioredis`, use disposable probe files outside git-tracked paths by default. Do not add one-off probes, harnesses, benchmarks, or examples under `examples/`, `lib/`, `test/`, or other repository directories unless the user explicitly asks for a checked-in artifact.
...
- In `ioredis`, default to a light local loop for probe authoring: temporary `probe.ts` plus temporary `tsconfig.json`, `npx tsc --noEmit -p <tmp-tsconfig>`, then `TS_NODE_TRANSPILE_ONLY=true node -r ts-node/register <tmp-probe>`. Escalate to `npm run build` only when the runtime question is specifically about `built/`, emitted declarations, or packaged output.
- In `ioredis`, do not treat a request for runtime verification, benchmarking, or comparison as permission to add a reusable example, benchmark harness, package script, or checked-in sample. Those repository changes require explicit user intent.
- For probes that need a running Redis, remove setup ambiguity before attributing a negative result to client behavior:
  ...
  - Treat RESP2 and RESP3 as separate cases, not interchangeable setup details, when the question is about reply shapes or push messages.
  - Clear unsupported server-version or command options first so they do not invalidate the probe. Assume a Redis server with the correct version is running on the default host and port unless the task says otherwise; if sandbox access is blocked, ask for permission to access Redis instead of treating Redis as unavailable.
...
2. For `ioredis`, declare the allowed write scope before you do any implementation work. For a normal disposable probe, that means a temporary directory only.
...
- Run TypeScript probes from the repository root with `TS_NODE_TRANSPILE_ONLY=true node -r ts-node/register <tmp-probe>` when practical.
...
- Avoid accidental imports from a different checkout or from `/tmp/node_modules`. If the probe needs repository code, import it from an absolute path resolved from `process.cwd()` and a repo-relative path.
- Prefer current-branch `lib/` imports when the question is "what does this branch do now?" and prefer `built/` imports only when the question is specifically about packaged output after a build.
- Do not run a repository-wide build just to typecheck a disposable probe. Reserve `npm run build` for `built/` probes or when emitted output is itself part of the question.
...
4. If the quick typecheck passes, run `TS_NODE_TRANSPILE_ONLY=true node -r ts-node/register "$tmpdir/probe.ts"`
5. Only run `npm run build` if the probe intentionally imports `built/` or validates packaged output
...
- Use `TS_NODE_TRANSPILE_ONLY=true node -r ts-node/register /tmp/probe.ts` from the repository root when practical.
- Use `npx tsc --noEmit -p /tmp/probe-tsconfig.json` as the default quick typecheck step before executing the probe.
- Import repository modules with an absolute path built from `process.cwd()` and a repo-relative path. Do not assume bare imports such as `ioredis` will resolve to the current checkout from `/tmp`.
- Use `lib/` imports for current-branch behavior probes and `built/` imports for packaged-output probes.
...
- Open [redis-runtime-patterns.md](./references/redis-runtime-patterns.md) for recurring ioredis runtime probe patterns.
