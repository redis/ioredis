...
/**
 * Disposable probe workflow for ioredis:
 * 1. Put this file in a temporary directory created with `mktemp -d`.
 * 2. Add a sibling `tsconfig.json` that extends `${process.cwd()}/tsconfig.json`
 *    and includes only `./probe.ts`.
 * 3. From the repository root, run `npx tsc --noEmit -p /tmp/.../tsconfig.json`.
 * 4. If that passes, run `TS_NODE_TRANSPILE_ONLY=true node -r ts-node/register /tmp/.../probe.ts`.
 * 5. Only switch to `built/` imports and `npm run build` when packaged output is the thing under test.
 */
  ...
  const summarizedCases = Object.fromEntries(
    [...cases.entries()].map(([caseId, items]) => {
      ...
      return [
        caseId,
        {
          mode: items[items.length - 1]?.mode ?? MODE,
...
async function main(): Promise<number> {
  ...
  // Example:
  // const client = await importRepoModule('lib/index.ts');
  // const Redis = (client.default ?? client) as unknown;
  // const hasRedisConstructor = typeof Redis === 'function';
  // recordCaseResult(
  //   caseId,
  //   hasRedisConstructor
  //     ? 'Loaded ioredis source from the repository root.'
  //     : 'ioredis source loaded but expected Redis constructor was missing.',
  //   hasRedisConstructor ? 'expected' : 'negative',
  //   { metrics: { import_path: 'lib/index.ts' } },
  // );
  recordCaseResult(
    caseId,
