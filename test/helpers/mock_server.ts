...
export default class MockServer extends EventEmitter {
  ...
  constructor (private port: number, public handler?: MockServerHandler, private slotTable?: any) {
    super()
    this.connect()
    ...
    this.socket = createServer((c) => {
      ...
      var parser = new Parser({
        ...
        returnReply: (reply: any) => {
          ...
          if (this.slotTable && reply.length === 2 && reply[0].toLowerCase() === 'cluster' && reply[1].toLowerCase() === 'slots') {
  ...
  broadcast(data: any) {
    this.clients.filter(c => c).forEach(client => {
  ...
  findClientByName(name: string): Socket | undefined {
    return this.clients.filter(c => c).find(client => {
