...
describe("readyHandler state restoration", () => {
  let savedListeners: any[] = [];
  let unhandled: string[] = [];

  beforeEach(() => {
    unhandled = [];
    // test/helpers/global.ts installs an unhandledRejection listener that throws, which
    // would turn a rejection here into an uncaught exception attributed to whichever test
    // happens to be running. Swap it out for a recorder and restore it afterwards.
  ...
  // Rejects `commandToReject` only from the second connection onwards, so the first
  // connection establishes state normally and only the post-reconnect replay fails.
  function serverRejectingReplayOf(
    ...
    const server = new MockServer(port, (argv) => {
      ...
      if (name === "subscribe" || name === "psubscribe") {
        return pubSubReply(2, name as any, String(argv[1]), 1);
      }
      return "OK";
    });
  ...
  async function reconnectAndCollect(redis: any, server: MockServer) {
    ...
    // Drop the socket so ioredis reconnects and readyHandler replays the state.
  ...
  it("surfaces a failing PSUBSCRIBE replay as an error event", async () => {
    ...
    const server = serverRejectingReplayOf(17_901, "psubscribe", message);
    const redis = client(17_901);
    await redis.connect();
  ...
  it("surfaces a failing SELECT replay before resubscribing as an error event", async () => {
    // condition.select resets to options.db (0) on reconnect while prevCondition.select
    // is 2, so readyHandler re-selects the db before replaying the subscriptions.
    const server = serverRejectingReplayOf(17_902, "select", NOPERM_SELECT);
    const redis = client(17_902);
    await redis.connect();
  ...
  it("surfaces a failing SELECT replay on a non-subscriber connection as an error event", async () => {
    // Same restoration, but through the final `select` in readyHandler rather than the
    // subscriber branch.
    const server = serverRejectingReplayOf(17_903, "select", NOPERM_SELECT);
    const redis = client(17_903);
    await redis.connect();
    await redis.select(2);
