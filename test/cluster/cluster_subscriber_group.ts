...
describe("cluster:ClusterSubscriberGroup", () => {
  ...
  it("works when ssubscribe only works for keys that map to the same slot", async () => {
    ...
    cluster
      .ssubscribe("channel{my}:1", "channel{yours}:2")
      .then((count: number) => {
    ...
    cluster
      .spublish("channel{my}:2", "This is a test message to my second channel.")
      .then((value: number) => {
    ...
    cluster
      .spublish(
        "channel{yours}:2",
        "This is a test message to your second channel."
      )
      .then((value: number) => {
  ...
  it("works when you can receive published messages to all primary nodes after having subscribed", async () => {
    ...
    for (const c of channels) {
      ...
      const numSubscribers = await publisher.spublish(
        c,
        "This is a test message  " + c + "."
      );
  ...
  it("receives messages on the channel after the slot was moved", async () => {
    ...
    await publisher.spublish(
      channel,
      "This is a test message #1 to slot " +
        slot +
        " on channel " +
        channel +
        "."
    ...
    const numSubscribers = await publisher.spublish(
      channel,
      "This is a test message #2 to slot " +
        slot +
        " on channel " +
        channel +
        "."
