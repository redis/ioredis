...
describe('connection', function () {
  ...
  describe('connectTimeout', () => {
    ...
    it('should clear the timeout when connected', function (done) {
      ...
      // TODO: use spy
      // @ts-ignore
      const stub = sinon.stub(Socket.prototype, 'setTimeout').callsFake(timeout => {
    ...
    it('should ignore timeout if connect', function (done) {
      ...
      // TODO: use spy
      // @ts-ignore
      sinon.stub(Socket.prototype, 'setTimeout').callsFake((timeout, callback) => {
        if (timeout === 0) {
          if (!isReady) {
            isReady = true
          } else {
  ...
  describe('#connect', function () {
    ...
    it('should stop reconnecting when disconnected', function (done) {
      ...
      redis.on('close', function () {
        redis.disconnect();
        sinon.stub(Redis.prototype, 'connect').throws(new Error('`connect` should not be called'));
        setTimeout(function () {
