/*
 * Vendored from `cluster-key-slot@1.1.1`
 ...
 * and keeps scanning for the next `}`, hashing whatever sits between -- which
 * can silently misroute commands. Vendoring was necessary because the
 * upstream repository has been archived and is no longer maintained, and
 * ioredis pins the dependency to an exact version rather than a range. See
 * https://github.com/mishan/intraslot/blob/main/contrib/empty-hashtag-slot-bug/README.md
 * for the full writeup, live-server measurements, and suggested test cases.
 *
 ...
 */
function generate(str: string | ArrayLike<number>): number {
  let char: number;
  let i = 0;
  let start = -1;
  ...
  while (i < len) {
    ...
    if (done || start === -1) {
      ...
      resultHash =
        lookup[(char ^ (resultHash >> 8)) & 0xff] ^ (resultHash << 8);
    } else if (i - 1 !== start) {
...
interface CalculateSlot {
  (str: string | ArrayLike<number>): number;
  /**
   * Convert an array of multiple strings into a redis slot hash.
   * Returns -1 if one of the keys is not for the same slot as the others
   */
  generateMulti(keys: (string | ArrayLike<number>)[]): number;
}

const calculateSlot = generate as CalculateSlot;

calculateSlot.generateMulti = function generateMulti(
  keys: (string | ArrayLike<number>)[]
): number {
  let i = 1;
  const len = keys.length;
  const base = generate(keys[0]);
