...
Cluster.prototype.selectSubscriber = function () {
  ...
  if (this.lastActiveSubscriber && this.lastActiveSubscriber.prevCondition && this.lastActiveSubscriber.prevCondition.subscriber) {
    var pending = 0;
    var subscribeChannels = this.lastActiveSubscriber.prevCondition.subscriber.channels('subscribe');
    if (subscribeChannels.length) {
      pending += 1;
      debug('subscribe %d channels', subscribeChannels.length);
      this.subscriber.subscribe(subscribeChannels).then(function () {
        if (!--pending) {
          this.lastActiveSubscriber = this.subscriber;
        }
      }.bind(this)).catch(noop);
    }
    var psubscribeChannels = this.lastActiveSubscriber.prevCondition.subscriber.channels('psubscribe');
    if (psubscribeChannels.length) {
      pending += 1;
      debug('psubscribe %d channels', psubscribeChannels.length);
      this.subscriber.psubscribe(psubscribeChannels).then(function () {
        if (!--pending) {
          this.lastActiveSubscriber = this.subscriber;
        }
      }.bind(this)).catch(noop);
