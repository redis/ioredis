...
export class Decoder {
  ...
  #decodeTypeValue(type, chunk) {
    switch (type) {
      ...
      case RESP_TYPES.NUMBER:
        return this.#handleDecodedValue(
          this.onReply,
          this.#decodeNumber(this.getTypeMapping()[RESP_TYPES.NUMBER], chunk)
        );
      ...
      case RESP_TYPES.DOUBLE:
        return this.#handleDecodedValue(
          this.onReply,
          this.#decodeDouble(this.getTypeMapping()[RESP_TYPES.DOUBLE], chunk)
        );
  ...
  #decodeVerbatimStringFormat(stringLength, chunk) {
    ...
    return this.#cursor >= chunk.length
      ? this.#continueDecodeVerbatimStringFormat.bind(
          this,
          stringLength,
          formatCb
        )
    ...
    switch (type) {
      ...
      case RESP_TYPES.SIMPLE_STRING:
        return this.#decodeSimpleString(
          typeMapping[RESP_TYPES.SIMPLE_STRING],
          chunk
        );

      case RESP_TYPES.BLOB_STRING:
        return this.#decodeBlobString(
          typeMapping[RESP_TYPES.BLOB_STRING],
          chunk
        );

      case RESP_TYPES.VERBATIM_STRING:
        return this.#decodeVerbatimString(
          typeMapping[RESP_TYPES.VERBATIM_STRING],
          chunk
        );
  ...
  #continueDecodeMapAsObjectValue(
    object,
    remaining,
    key,
    valueCb,
