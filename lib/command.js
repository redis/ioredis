...
Command.setArgumentTransformer('zadd', function (args) {
  if (args.length > 1) {
    var last = args.pop();
    if (typeof Map !== 'undefined' && last instanceof Map) {
      return args.concat(utils.convertMapToInvertArray(last));
    }
    if (typeof last === 'object' && last !== null) {
      return args.concat(utils.convertObjectToInvertArray(last));
    }
    args.push(last);
  }
  return args;
