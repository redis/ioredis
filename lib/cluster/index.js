...
var NODE_STATUS = utils.NODE_STATUS;
...
function Cluster(startupNodes, options) {
  ...
  this.connectionPool.on('drain', function () {
    _this.setStatus(NODE_STATUS.CLOSE);
  });
  ...
  if (this.options.lazyConnect) {
    this.setStatus(NODE_STATUS.WAIT);
  } else {
...
Cluster.prototype.connect = function () {
  ...
  return new Promise(function (resolve, reject) {
    if (this.status === NODE_STATUS.CONNECTING
        || this.status === NODE_STATUS.CONNECT
        || this.status === NODE_STATUS.READY) {
      reject(new Error('Redis is already connecting/connected'));
      return;
    }
    this.setStatus(NODE_STATUS.CONNECTING);
    ...
    function readyHandler() {
      this.setStatus(NODE_STATUS.READY);
      this.retryAttempts = 0;
    ...
    var readyCheckCallback = function (err, fail) {
      if (err || fail) {
        debug('Ready check failed (%s). Reconnecting...', err || fail);
        if (this.status === NODE_STATUS.CONNECT) {
    ...
    var refreshListener = function () {
      ...
      this.setStatus(NODE_STATUS.CONNECT);
      if (this.options.enableReadyCheck) {
        this.connectionPool.onReady(() => {
          this._readyCheck(readyCheckCallback);
        });
      } else {
        readyHandler.call(this);
      }
...
Cluster.prototype._handleCloseEvent = function () {
  ...
  if (typeof retryDelay === 'number') {
    this.setStatus(NODE_STATUS.RECONNECTING);
    ...
    this.setStatus(NODE_STATUS.END);
...
Cluster.prototype.disconnect = function (reconnect) {
  ...
  this.setStatus(NODE_STATUS.DISCONNECTING);
  ...
  if (status === NODE_STATUS.WAIT) {
    this.setStatus(NODE_STATUS.CLOSE);
...
Cluster.prototype.quit = function (callback) {
  ...
  if (status === NODE_STATUS.WAIT) {
    ...
    setImmediate(function (ctx) {
      ctx.setStatus(NODE_STATUS.CLOSE);
      ctx._handleCloseEvent();
    }, this);
...
Cluster.prototype.selectSubscriber = function () {
  ...
  if (previousChannels.subscribe.length || previousChannels.psubscribe.length) {
    ...
    if (this.subscriber.status === NODE_STATUS.WAIT) {
...
Cluster.prototype.refreshSlotsCache = function (callback) {
  ...
  function tryNode(index) {
    ...
    _this.getInfoFromNode(_this.connectionPool.nodes.all[keys[index]], function (err) {
      if (_this.status === NODE_STATUS.END) {
...
Cluster.prototype.sendCommand = function (command, stream, node) {
  if (this.status === NODE_STATUS.WAIT) {
    this.connect().catch(_.noop);
  }
  if (this.status === NODE_STATUS.END) {
  ...
  function tryConnection(random, asking) {
    ...
    var redis;
    if (_this.status === NODE_STATUS.READY || (command.name === 'cluster')) {
      if (node && node.redis) {
        redis = node.redis;
