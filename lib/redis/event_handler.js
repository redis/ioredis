...
exports.connectHandler = function (self) {
  return function () {
    if (self.stream.remoteAddress && self.stream.remoteAddress !== self.options.host) {
      self.options.host = self.stream.remoteAddress;
      debug('updated host', self.options.host);
    }

    if (self.stream.remotePort && self.stream.remotePort !== self.options.port) {
      self.options.port = self.stream.remotePort;
      debug('updated host', self.options.port);
    }

    self.setStatus(NODE_STATUS.CONNECT);
...
exports.closeHandler = function (self) {
  return function () {
    self.setStatus(NODE_STATUS.CLOSE);
    ...
    self.setStatus(NODE_STATUS.RECONNECTING, retryDelay);
  ...
  function close() {
    self.setStatus(NODE_STATUS.END);
...
exports.readyHandler = function (self) {
  return function () {
    self.setStatus(NODE_STATUS.READY);
    ...
    if (self.options.monitor) {
      ...
      self.setStatus(NODE_STATUS.MONITORING);
      return;
    }
    var item;
