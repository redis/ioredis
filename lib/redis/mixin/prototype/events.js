...
function readyHandler(self) {
  return function () {
    ...
    if (self.condition.mode.monitoring) {
      ...
      // TODO detect run id
      _.keys(self.scriptsSet).forEach(function (name) {
        debug('load custom command: %s', name);
        var script = self.scriptsSet[name];
        self.sendCommand(script.getLoadCommand());
      });

      debug('send commands in offline queue: %d', self.offlineQueue.length);
      for (var i = 0; i < self.offlineQueue.length; ++i) {
        self.sendCommand(self.offlineQueue[i][0], self.offlineQueue[i][1]);
      }
      self.offlineQueue = [];
