  ...
  if (writable) {
    ...
    if (isSubscribeCommand || isUnsubscribeCommand) {
      ...
      for (var i = 0; i < command.args.length; ++i) {
        var channel = command.args[i];
        if (isSubscribeCommand) {
          this.subscriptionSet[channel] = true;
        } else {
          delete this.subscriptionSet[channel];
        }
        if (!this.subscriptionQueue[command.name]) {
          this.subscriptionQueue[command.name] = {};
        }
        if (!this.subscriptionQueue[command.name][channel]) {
          this.subscriptionQueue[command.name][channel] = [];
        }
        this.subscriptionQueue[command.name][channel].push(command);
      }
      command.remainReply = command.args.length;
    } else {
