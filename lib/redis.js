...
function Redis(port, host, options) {
  ...
  if (typeof port === 'object') {
    ...
    if (parsedURL.hostname) {
      ...
      if (parsedURL.auth) {
        parsedOptions.auth = parsedURL.auth.split(':')[1];
      }
      if (parsedURL.path) {
        parsedOptions.select = parseInt(parsedURL.path.slice(1), 10);
      }
    } else {
...
Redis.prototype.sendCommand = function (command, stream) {
  ...
  var writable = this.connection.writable && ((this.status === 'ready') || ((this.status === 'connected') &&  _.includes(Command.FLAGS.VALID_WHEN_LOADING, command.name)));
  if (writable) {
    debug('write command -> %s(%s)', command.name, command.args);
...
_.assign(Redis.prototype, require('./redis/prototype/events'));
