 ...
 * @param {number} [options.retryTime] - when redis server is not ready, we will wait for
 * retryTime (milliseconds). By default, ioredis will use `loading_eta_seconds` from `info` command.
...
Redis.prototype._readyCheck = function (callback) {
  ...
  this.info(function (err, res) {
    ...
    if (!info.loading || info.loading === '0') {
      ...
      var retryTime = _this.options.retryTime || ((info.loading_eta_seconds || 1) * 1000);
