...
Redis.prototype.connect = function () {
  console.log('====', this._sentinel);
  if (this._sentinel) {
    debug('init sentinels');
    var _this = this;
    this._sentinel.once('connect', function (connection) {
      debug('received connection from sentinel');
      _this.connection = connection;
      _this.connection.once('connect', eventHandler.connectHandler(_this));
      _this.connection.once('error', eventHandler.errorHandler(_this));
      _this.connection.once('close', eventHandler.closeHandler(_this));
      _this.connection.on('data', eventHandler.dataHandler(_this));
    });
    ...
    this.connection.once('connect', eventHandler.connectHandler(this));
    this.connection.once('error', eventHandler.errorHandler(this));
    this.connection.once('close', eventHandler.closeHandler(this));
    this.connection.on('data', eventHandler.dataHandler(this));
  }
};
