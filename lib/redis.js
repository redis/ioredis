...
Redis.prototype.flushQueue = function (error) {
  var command;
  for (var i = 0; i < this.offlineQueue.length; ++i) {
    command = this.offlineQueue[i];
...
Redis.prototype.defineCommand = function (name, definition) {
  ...
  this.sendCommand(script.getLoadCommand());
  ...
  this[name] = function () {
    ...
    var command;
    try {
      command = script.getExecuteCommand(args, 'utf8', callback);
    } catch (err) {
  ...
  this[name + 'Buffer'] = function () {
    ...
    var command;
    try {
      command = script.getExecuteCommand(args, null, callback);
    } catch (err) {
      return Promise.reject(err).nodeify(callback);
    }
    return this.sendCommand(command);
...
Redis.prototype.undefineCommand = function (name, definition) {
  var script = new Script(definition.lua, definition.numberOfKeys);
  this.sendCommand(script.getLoadCommand());
  this.scriptsSet[name] = script;
  // TODO protect property.
};

Redis.prototype.loadCustomCommands = function () {
  for (var key in this.scriptsSet) {
    if (this.scriptsSet.hasOwnProperty(key)) {
      this.sendCommand(this.scriptsSet[key].getLoadCommand());
    }
  }
};
