  ...
  var promise = new Promise(function (resolve, reject) {
    ...
    this.connector.connect(function (err, stream) {
      if (err) {
        ...
        reject(err);
        _this.setStatus(NODE_STATUS.END);
        return;
      }
      ...
      var connectionCloseHandler = function () {
        _this.removeListener(NODE_STATUS.READY, connectionReadyHandler);
        reject(new Error(utils.CONNECTION_CLOSED_ERROR_MSG));
      };
      _this.once(NODE_STATUS.READY, connectionReadyHandler);
...
Redis.prototype.sendCommand = function (command, stream) {
  ...
  var writable = (this.status === NODE_STATUS.READY) ||
    (!stream && (this.status === NODE_STATUS.CONNECT) && commands.hasFlag(command.name, 'loading'));
  if (!this.stream) {
    writable = false;
